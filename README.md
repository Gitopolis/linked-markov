...
fn get_or_insert<'a>(dict: &Arc<RwLock<HashMap<&'a str, ToStep<&'a str>>>>, w: &'a str) -> ToStep<&'a str> {
    // Fast path: read lock
    if let Some(existing) = { dict.read().unwrap().get(w).cloned() } {
        return existing;
    }
    // Slow path: write lock with entry to avoid races
    let mut map = dict.write().unwrap();
    map.entry(w)
        .or_insert(ToStep::new(Step::new(w)))
        .clone()
}
...
fn main() {
    ...
    for sonnet in SONNETS.iter().cloned() {
        ...
        threads.push(thread::spawn(move || {
            let mut words = sonnet.split_whitespace();
            let Some(first) = words.next() else { return };
            let mut current = get_or_insert(&dict, first);

            for word in words {
                // word is &'static str because sonnet is &'static str
                let word: &'static str = word;
                let next = get_or_insert(&dict, word);
                {
                    let mut trans = current.transitions.write().unwrap();
                    *trans.entry(next.clone()).or_insert(0) += 1;
                }
                current = next;
            }
        }));
